...
## import light location, color and radius 
    ...
    while i < len(lineArray):
        ...
        ## maya coordinate system is not synced with game - maya is 10x larger
        Location = [Location[0]*10,Location[1]*10,Location[2]*-10]
        Scale = [lineArray[i][6]*10,lineArray[i][6]*10,lineArray[i][6]*10]

        ## Create maya lights
        activeLight = cmds.pointLight(position = (Location),rgb = (Color))
        cmds.scale(Scale[0],Scale[1],Scale[2])
        activeRadius = cmds.sphere(radius = (Scale[0]))
        cmds.move(Location[0],Location[1],Location[2], activeRadius)
        radiusName = ''
        radiusName = str(activeRadius[0])+'.template'
        cmds.setAttr(radiusName, 1)
        
        aName = activeLight.split('Shape',2)
        sName = ''
        sName = aName[0] + aName[1]
        cmds.parent(activeRadius[0],sName)
        i+=1
else:
...
for light in Lights:
    ...
    shuffleLight[0] = int(currLight[3])/10
    shuffleLight[1] = int(currLight[4])/10
    shuffleLight[2] = int(currLight[5])/-10
